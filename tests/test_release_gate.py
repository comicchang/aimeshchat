...
def _remote_exec_available() -> bool:
    """Return True if aimeshchat-remote-exec is accessible via localhost SSH."""
    if not _localhost_ssh_available():
        return False
    # Check if aimeshchat-remote-exec is on the SSH session PATH
    ...
    try:
        r = subprocess.run(
            ["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=3",
             "localhost",
             "export PATH=$HOME/.local/bin:$PATH; aimeshchat-remote-exec --help"],
            capture_output=True, timeout=10,
        )
        if r.returncode == 0:
            return True
...
requires_localhost_ssh = pytest.mark.skipif(
    ...
    reason="localhost SSH or aimeshchat-remote-exec not available",
)
