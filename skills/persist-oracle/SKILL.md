---
name: persist-oracle
description: 持久化多轮 Oracle review — 保留上下文。仅用 codeagent oracle start/ask/status/watch/release。OMP 用 omp-config memory + parked-revive，OpenCode 用原生 --session 续接；oh-my-openagent 不加额外 session 字段。
---
...
codeagent oracle start "$KEY" --agent oracle --prompt '初始问题'

# 追加/追问：hot in-loop send（同 backend session，不新开进程）
codeagent oracle ask "$KEY" '追加信息'

# 状态：聚合 receipt / progress / park / runtime health
codeagent oracle status "$KEY"

# 进度：cursor 可续的事件流
codeagent oracle watch "$KEY" --cursor <last>

# 终止：写 terminal + 停 runtime + 释放 park（唯一终止途径）
codeagent oracle release "$KEY"
```
