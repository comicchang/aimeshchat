...
**v2 control plane（跨设备 runtime）优先**：Manager 先用 `aimeshchat gateway ensure --host <H>` 预检（wire v2 / tmux / 架构）并启动远端 gateway，然后 session.ensure / runtime.spawn / runtime.send 走 SSH ControlMaster 单次 `gateway rpc --stdio`；mailbox + RuntimeEvent 走长期 SSHStream 回流。远端永不反向连接 Manager、不开放端口。

**leaf transport（纯 mailbox 消息）**：`aimeshchat mailbox <subcommand> ... --host <H>` 仍是跨主机 mailbox 消息传输原语。

### What IS the transport

| Command | Scope | Notes |
|---|---|---|
| `aimeshchat mailbox read --session <id> --agent <a> --owner <a> --host <H>` | cross-host | Manager reads Worker's host-local inbox |
| `aimeshchat mailbox peek --session <id> --agent <a> --host <H>` | cross-host | non-destructive inbox count |
| `aimeshchat mailbox stats --session <id> --agent <a> --host <H>` | cross-host | 4-dir stats (inbox/processing/archive/_corrupt) |
| `aimeshchat mailbox send --host <H> ...` | cross-host | push a message to remote host's local mailbox CLI |
| `aimeshchat mailbox stats --session <id> --agent <a> --host <H>` | cross-host | read inbox/processing/archive/_corrupt counts (read-only) |
| `aimeshchat mailbox status --session <id> --agent <a> --host <H> ...` | cross-host | **write-only** status update (IDLE/BUSY/DONE/BLOCKED); not a read command |
| `aimeshchat swarm direct <s> --from X --to <a> --kind TASK ...` | local | SessionManifest-aware routing, local kernel delivery |

### What does NOT exist (never use)

| Phantom command | Real replacement | Why it fails |
|---|---|---|
| `aimeshchat swarm send ...` | `aimeshchat swarm direct ...` | `send` is not a subcommand; `direct` is the real dispatch command |
| `aimeshchat swarm poll --session ... --host <H>` | `aimeshchat mailbox read --host <H>` | `poll` is local-only; no `--host` flag |
| `aimeshchat swarm status --host <H>` | `aimeshchat mailbox stats --host <H>` | `status` is local-only; use `mailbox stats` for cross-host counts |
| `aimeshchat manager-poll` | `aimeshchat mailbox read --host <H>` | not a real subcommand |
| `aimeshchat swarm status --all-hosts` | iterate hosts with `mailbox stats --host <H>` | not a real subcommand |
...
- Both sides use `aimeshchat mailbox ... --host <H>`.
...
aimeshchat mailbox read --session <session-id> --agent manager --owner manager --host <H> --json
# ... verify report ...
# Finalize on remote host
aimeshchat mailbox finalize --session <session-id> --agent manager --msg-id <id> --owner manager --host <H>
```
...
aimeshchat mailbox stats --session <session-id> --agent <worker-id> --host <worker-host>
```
...
**Mailbox path resolution**: Worker uses bare `mailbox` CLI (local FS, no `--host`) for all reads, writes, and status updates on its own host. Manager uses `aimeshchat mailbox ... --host <H>` for all cross-host operations to the Worker's host. Never use `--host` on the Worker side or omit it on the Manager side when crossing host boundaries.
...
- [ ] **CLI installed**: `ssh <worker-host> "which aimeshchat && aimeshchat --version"` returns expected version
...
> **Pre-conditions**: All commands below are verified against the installed CLI (`aimeshchat 0.2.8+`).
...
> Remote hosts MUST have `aimeshchat` in `PATH` and mailbox root directories writable by the SSH user.
...
aimeshchat swarm create-session "$SID" \
  --manager manager \
...
aimeshchat swarm register "$SID" --agent w-frontend        --host H1 --backend cli
aimeshchat swarm register "$SID" --agent w-backend         --host H1 --backend cli
aimeshchat swarm register "$SID" --agent w-reverse         --host H2 --backend cli
aimeshchat swarm register "$SID" --agent w-docs            --host H2 --backend cli
aimeshchat swarm register "$SID" --agent w-remote-analysis --host H3 --backend omp
```
...
aimeshchat mailbox session-init --session "$SID" --manager manager \
...
for H in H1 H2 H3; do
  ssh "$H" "aimeshchat --version" || echo "FAIL: $H missing aimeshchat"
done
...
for H in H1 H2; do
  for W in w-frontend w-backend w-reverse w-docs; do
    ...
    echo "=== $W on $H ==="
    aimeshchat mailbox stats --session "$SID" --agent "$W" --host "$H"
  done
done
...
aimeshchat mailbox read --session "$SID" --agent manager --owner manager --host H1 --json
aimeshchat mailbox read --session "$SID" --agent manager --owner manager --host H2 --json

# 3. Verify artifacts (if REPORT includes attachment refs)
# aimeshchat artifact pull --host H1 --artifact-id <id> \
#   --relative-path output/result.tar.gz --size <bytes> --sha256 <hex> --dest ./artifacts/

# 4. Finalize consumed messages on each host
aimeshchat mailbox finalize --session "$SID" --agent manager --msg-id <report-msg-id> --owner manager --host H1
aimeshchat mailbox finalize --session "$SID" --agent manager --msg-id <report-msg-id> --owner manager --host H2
```
...
aimeshchat mailbox finalize --session "$SID" --agent manager --msg-id <id> --owner manager --host H1
```
