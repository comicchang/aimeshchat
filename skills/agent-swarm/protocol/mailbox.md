...
1. Fetch each artifact via `aimeshchat artifact pull --host <source_host> --artifact-id <artifact_id> --remote-root <remote_root> --relative-path <relative_path> --size <size> --sha256 <sha256> --dest <dest>` (the real cross-host transport).
...
2. `aimeshchat mailbox` as unified cross-host entry point
3. For swarm sessions: `aimeshchat swarm ...` subcommands
