...
1. CWD 是否已在目标仓内？
   是 → 直接本地工具
2. codeagent route where <topic> 能否找到映射？
   否 → 本地文档或先补配置
   是 → 继续
...
codeagent route where <topic>
printf '%s\n' '<task>' | codeagent route <topic> --repo 0 --backend codex --agent explore
codeagent route <topic> '<task>' --dry-run

# 直接执行
codeagent run '<task>' <workdir> --host <host> --backend codex --agent develop

# Session 管理
codeagent sessions list [--host H] [--topic T]
codeagent sessions show '<key>'
codeagent sessions reset '<key>'
codeagent sessions bind --key '<key>' --id '<backend-session-id>'
...
| `python3 code_route.py where T` | `codeagent route where T` |
| `echo TASK \| python3 code_route.py route T --backend B` | `printf '%s\n' TASK \| codeagent route T --backend B` |
